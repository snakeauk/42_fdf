...
int	mlx_int_get_color_text_rgb(char *name, char *end)
{
	int			i;
	char		buff[64];
...
void	*mlx_int_parse_xpm(t_xvar *xvar,void *info,int info_size,char *(*f)())
{
		...
		while (i--)
		{
				...
				if (!tab[j])
						RETURN;
				rgb_col = mlx_int_get_color_text_rgb(tab[j], tab[j+1]);
				/*
				if ((rgb_col = mlx_int_get_color_text_rgb(tab[j], tab[j+1]))==-1)
				{
...
int	mlx_int_file_get_color_rid_comment(char *ptr, int size)
{
		int	com_begin;
		int	com_end;
...
void	*mlx_xpm_file_to_image(t_xvar *xvar,char *file,int *width,int *height)
{
		...
		mlx_int_file_get_color_rid_comment(ptr, size);
